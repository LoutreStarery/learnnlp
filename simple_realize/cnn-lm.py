...
class Seq2Seq(keras.Model):
    ...
    def build(self, input_shape):
        ...
        self.conv2ds = [
            keras.layers.Conv2D(16, (n, self.emb_dim), padding="valid", activation=keras.activations.relu)
            for n in range(2, 5)]
        self.max_pools = [keras.layers.MaxPool2D((n, 1)) for n in [7, 6, 5]]
        self.encoder = keras.layers.Dense(self.units, activation=keras.activations.relu)
...
class myTensorboard(keras.callbacks.TensorBoard):
    def __init__(self, data, log_dir='logs/cnn-lm', histogram_freq=1, write_graph=True, write_images=True,
